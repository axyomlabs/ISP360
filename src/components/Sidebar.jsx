...
import {
  FaHome, FaUsers, FaNetworkWired, FaHeartbeat, FaHeadset, FaFileInvoice,
...
const SidebarItem = ({ item }) => {
  if (!item.children) {
    return (
      <a href={item.link} className="text-white text-decoration-none d-flex align-items-center mb-2">
  ...
  return (
    <div className="mb-2">
      <a
        ...
        aria-expanded="false"
      >
        <span>{item.icon}<span className="ms-2">{item.title}</span></span>
        <FaChevronDown className="transition-arrow" />
      </a>
      <div className="collapse ps-4" id={item.id} data-bs-parent="#sidebarMenu">
        {item.children.map((child, i) => (
          <a key={i} href={child.link} className="d-block text-white text-decoration-none py-1">
            {child.title}
          </a>
        ))}
      </div>
